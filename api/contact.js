...
function createStyledEmail(content) {
    ...
    return `
    <!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${title}</title><style>body { font-family: 'Inter', Arial, sans-serif; }</style></head><body style="margin: 0; padding: 0; background-color: ${sandBeige}; font-family: Inter, Arial, sans-serif;"><div style="display: none; max-height: 0; overflow: hidden;">${preheader}</div><table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; margin: 20px auto; background-color: #FFFFFF; border: 1px solid #E6E2DB; border-radius: 16px; box-shadow: 0 8px 32px rgba(45, 42, 37, 0.06);"><tr><td align="center" style="padding: 20px 0;"><img src="https://www.wizmanheritage.com/Logo_WizmanHeritage.png" alt="WizmanHeritage Logo" width="180" style="display: block;"></td></tr><tr><td style="padding: 30px; color: ${textPrimary};">${body_content}</td></tr><tr><td align="center" style="padding: 20px 30px; background-color: #FEFCF8; border-top: 1px solid #E6E2DB; border-bottom-left-radius: 16px; border-bottom-right-radius: 16px;"><p style="margin: 0; color: ${textSecondary}; font-size: 12px;">${footer_text}</p></td></tr></table></body></html>`;
}

const confirmationContent = {
    fr: { title: "Confirmation de votre demande | WizmanHeritage", preheader: "Nous avons bien reçu votre demande et revenons vers vous rapidement.", greeting: "Bonjour {name},", main_text: "Nous avons bien reçu votre demande et nous vous remercions de votre confiance.<br><br>Un membre de notre cabinet l'examinera avec la plus grande attention. <strong>WizmanHeritage s'engage à vous répondre dans un délai de 48 heures ouvrées.</strong><br><br>Votre demande est traitée avec la plus stricte confidentialité.", closing: "Cordialement,", team_name: "L'équipe WizmanHeritage", footer_text: "&copy; 2024 WizmanHeritage. Tous droits réservés. Cet email est une confirmation de réception." },
    en: { title: "Request Confirmation | WizmanHeritage", preheader: "We have received your request and will get back to you shortly.", greeting: "Dear {name},", main_text: "We have successfully received your request and thank you for your trust.<br><br>A member of our firm will review it with the utmost care. <strong>WizmanHeritage commits to responding within 48 business hours.</strong><br><br>Your request is being handled with the strictest confidentiality.", closing: "Sincerely,", team_name: "The WizmanHeritage Team", footer_text: "&copy; 2024 WizmanHeritage. All rights reserved. This email is a receipt confirmation." },
    he: { title: "אישור קבלת פנייתך | WizmanHeritage", preheader: "קיבלנו את פנייתך ונחזור אליך בהקדם.", greeting: "שלום {name},", main_text: "פנייתך התקבלה בהצלחה, ואנו מודים לך על אמונך.<br><br>נציג ממשרדנו יבחן את פנייתך בקפידה. <strong>WizmanHeritage מתחייבת להשיב לפנייתך תוך 48 שעות עסקים.</strong><br><br>פנייתך מטופלת בסודיות מוחלטת.", closing: "בכבוד רב,", team_name: "צוות WizmanHeritage", footer_text: "&copy; 2024 WizmanHeritage. כל הזכויות שמורות. אימייל זה מהווה אישור קבלה." }
};
...
export default async (req, res) => {
    ...
    try {
        ...
        const formattedClientFiles = clientFiles.map(f => ({ ...f, content: f.content.toString('base64') }));
        ...
        const notificationMsg = {
            // =================================================================
            // DERNIÈRE CORRECTION ICI : ENVOI AUX DEUX ADRESSES
            // =================================================================
