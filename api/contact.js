// api/contact.js — VERSION FINALE AVEC reCAPTCHA et logo PNG optimisé
...
export default async function handler(req, res) {
  ...
  try {
        ...
        bb.on('file', (fieldname, file, info) => {
          ...
          file.on('end', () => {
            files.push({
              ...
              disposition: 'attachment', _originalMime: mimeType,
            });
          });
        });
    ...
    const { fields, files: clientFiles } = await parseMultipartForm(req.headers, rawBody);
    
    const recaptchaToken = fields.recaptchaToken;
    if (!recaptchaToken) { return res.status(400).json({ message: 'reCAPTCHA token missing' }); }
    const isHuman = await verifyRecaptcha(recaptchaToken);
    if (!isHuman) { return res.status(403).json({ message: 'reCAPTCHA verification failed' }); }
    ...
    // === CORRECTION : Utilisation du nouveau logo PNG avec le bon nom de fichier ===
    const logoPath = path.join(process.cwd(), 'logo-wizmanheritage-genealogie-successorale.png');
    const logoBuffer = await fs.readFile(logoPath);
    const logoBase64 = logoBuffer.toString('base64');
    const logoAttachment = {
      content: logoBase64,
      filename: 'Logo_WizmanHeritage.png', // Le nom dans l'email peut rester simple
      type: 'image/png',
      disposition: 'inline',
      content_id: 'logo',
    };
    ...
    const ipAddress = (req.headers['x-forwarded-for'] || '').toString().split(',')[0] || 'Non disponible';
    ...
    const confirmationBody = `
      ...
      <br>
      <p style="font-size:16px;line-height:1.6;">${clientContent.closing}<br><strong>${clientContent.team_name}</strong></p>`;
