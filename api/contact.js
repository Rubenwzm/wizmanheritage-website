...
export default async (req, res) => {
    ...
    try {
        // CORRECTION MAJEURE: Adapter la gestion du corps de la requête pour Vercel
        // Vercel fournit le corps de la requête en mode "stream" ou "raw body" pour les requêtes multipart.
        // Nous lisons le corps entier manuellement.
        const requestBody = await new Promise((resolve, reject) => {
            let body = '';
            req.on('data', (chunk) => {
                body += chunk.toString('base64'); // Lisez le corps en base64
            });
            req.on('end', () => {
                resolve(body);
            });
        ...
        // La fonction parseMultipartForm est adaptée pour prendre ce corps lu.
        const parseMultipartForm = (headers, bodyBuffer) => { // Prend les headers et le buffer du corps
            return new Promise((resolve, reject) => {
                const fields = {};
                const files = [];
                const bb = busboy({ headers }); // Utilisez les headers passés en argument
                ...
                bb.end(bodyBuffer); // Passe le buffer directement
            });
        };

        const { fields, files: clientFiles } = await parseMultipartForm(req.headers, Buffer.from(requestBody, 'base64')); // Appel avec les headers et le buffer
        ...
        // Préparation des pièces jointes du client pour SendGrid
